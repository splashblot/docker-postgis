...
CREATE EXTENSION IF NOT EXISTS observatory WITH VERSION 'dev';
...
SELECT cartodb.cdb_conf_setconf('data_observatory_conf', '{"connection": {"whitelist": ["ethervoid"], "production": "host=postgis port=5432 dbname=dataservices_db user=geocoder_api", "staging": "host=postgis port=5432 dbname=dataservices_db user=geocoder_api"}, "monthly_quota": 100000}');
EOSQL
done
